overlap_geral_vi <- function(x,y,name,especie,tipo){
  ...
  for(f in 1:length(con)){
   ...
   if(f == 2){ 
    png(filename = paste0("figures/",especie,"/kernel_",tipo,"/", name,".png")) 
    plot(aa2, col = "red", main = name)
    lines(aa, col = "blue")
    dev.off()
    }
    ...
    for(d in 1:lengths(coordinates(aa))){
      for(a in 1:lengths(coordinates(aa2))){
        ...
        at1 <- c(at1,areas_poly[idx[1,1]])
        at2 <- c(at2,areas_poly[idx[2,1]])
